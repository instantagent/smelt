...
Agent has one hosted lint lane and several local test tiers. The split keeps
...
| Slow | `bash tools/test-slow.sh` | manual, run locally | Quantizer perf and quality sweeps, extended prefill kernel coverage. Sets `AGENT_RUN_SLOW_TESTS=1`. Hermetic — no external assets, just slow. |
| Maintainer | `AGENT_INCLUDE_MAINTAINER_TESTS=1 swift test` | release verification and module migration work | Private release-evidence harnesses, completion-matrix validation, selector-deletion scans, and CAM command/source migration lints. |
...
|---|---|---|
| `AGENT_RUN_SLOW_TESTS` | Quantizer perf + quality sweeps; large-batch prefill kernel coverage. | None — slow but hermetic. |
| `AGENT_INCLUDE_MAINTAINER_TESTS` | Private release-evidence, completion-matrix, selector-deletion, and source-migration suites. | None; `tools/verify-release.sh` enables this automatically. |
| `AGENT_RUN_QWEN_SMOKE` | Qwen 3.5 end-to-end smoke (prefill ↔ decode parity, prefix caching, GPU temperature sampling) and extended large-batch prefill coverage. | Built canonical Qwen packages. |
| `AGENT_RUN_EXPERIMENTAL_QMM_FFN_TESTS` | Experimental QMM-on-FFN tests. Subject to change. | None. |
...
| Variable | Effect |
|---|---|
| `AGENT_QWEN_PACKAGE` | Override path to the Qwen 2B canonical package. |
| `AGENT_QWEN_0_8B_PACKAGE` | Override path to the Qwen 0.8B canonical package. |
