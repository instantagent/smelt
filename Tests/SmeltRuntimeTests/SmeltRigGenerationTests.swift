...
@Suite("Smelt rig generation policy")
struct SmeltRigGenerationTests {
    @Test("Vocabulary constants match the pinned checkpoint")
    func vocabulary() {
        #expect(SmeltRigVocabulary.skinTokenRange.count == 32_768)
        #expect(SmeltRigVocabulary.skinTokenRange.first == 267)
        #expect(SmeltRigVocabulary.skinTokenRange.last == 33_034)
        #expect(SmeltRigVocabulary.modelEOS == 33_035)
        #expect(SmeltRigVocabulary.vocabularySize == 33_036)
    }

    @Test("Default generation budget uses all artifact context")
    func defaultGenerationBudget() {
        #expect(SmeltRigGenerationConfiguration().maximumGeneratedTokens == nil)
    }

    @Test("Validated skeleton choices preserve completion context")
    func completionBudget() throws {
        let oneJoint = [257, 263, 10, 20, 30]
        let closingOnly = try SmeltRigGenerationPolicy.allowedNextTokens(
            sequence: oneJoint,
            remainingTokenBudget: 12
        )
        #expect(closingOnly == [SmeltRigVocabulary.skeletonEOS])

        let oneMoreJoint = try SmeltRigGenerationPolicy.allowedNextTokens(
            sequence: oneJoint,
            remainingTokenBudget: 13
        )
        #expect(oneMoreJoint.contains(SmeltRigVocabulary.skeletonEOS))
        #expect(oneMoreJoint.contains(0))
        #expect(!oneMoreJoint.contains(SmeltRigVocabulary.spring))
        #expect(!oneMoreJoint.contains(SmeltRigVocabulary.branch))

        let partRecord = try SmeltRigGenerationPolicy.allowedNextTokens(
            sequence: oneJoint,
            remainingTokenBudget: 14
        )
        #expect(partRecord.contains(SmeltRigVocabulary.spring))

        let branchRecord = try SmeltRigGenerationPolicy.allowedNextTokens(
            sequence: oneJoint,
            remainingTokenBudget: 17
        )
        #expect(branchRecord.contains(SmeltRigVocabulary.branch))
    }
    ...
    func validatedSkinPhase() throws {
        ...
        #expect(
            try SmeltRigGenerationPolicy.allowedNextTokens(
                sequence: skeleton,
                mode: .validated
            ) == Array(267..<33_035)
        )
    ...
    func sourceCompatibleSkinPhase() throws {
        ...
        let initial = try SmeltRigGenerationPolicy.allowedNextTokens(
            sequence: skeleton,
            mode: .sourceCompatible
        )
    ...
    func sampling() throws {
        var logits = [Float](
            repeating: -.infinity,
            count: SmeltRigVocabulary.vocabularySize
        )
        logits[267] = 5
        logits[268] = 4.9
        logits[269] = 4.8
        let topOne = try SmeltRigGenerationPolicy.sample(
            logits: logits,
            history: [],
        ...
        let penalized = try SmeltRigGenerationPolicy.sample(
            logits: logits,
            history: [267],
            allowedTokens: [267, 268, 269],
            repetitionPenalty: 2,
        ...
        let nucleus = try SmeltRigGenerationPolicy.sample(
            logits: logits,
            history: [],
            allowedTokens: [267, 268, 269],
            repetitionPenalty: 1,
