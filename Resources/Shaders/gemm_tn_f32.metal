...
/// f32-weight variant of the direct multi-N batched GEMV gemm_tn_bf16w_f32 — see that file for
/// the shape rationale. BIT-EXACT per output (n,m) vs gemv_f32/gemm_f32 (same lane-strided chunk
/// order, one accumulator per n, one simd_sum).
///
...
kernel void gemm_tn_f32(
    ...
    for (uint c = lane; c < chunks; c += 32) {
        ...
        for (uint nn = 0; nn < GEMM_TN; ++nn) {
            uint n = n0 + nn;
            if (n < N) acc[nn] += dot(xv, W4[n * chunks + c]);
        }
    }
    #pragma unroll
    for (uint nn = 0; nn < GEMM_TN; ++nn) {
