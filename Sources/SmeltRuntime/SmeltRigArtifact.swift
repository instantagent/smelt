...
/// Validated, mmap-backed rig model component package.
public final class SmeltRigArtifact: CheckpointTensorSource {
    public let path: String
    public let manifest: SmeltRigPackageManifest
    ...
    public init(path: String, verify: Bool = true) throws {
        ...
        guard FileManager.default.fileExists(atPath: manifestURL.path) else {
            throw SmeltRigArtifactError.missingFile(manifestURL.path)
        }
        let manifest = try SmeltRigPackageManifest.decode(
        ...
        {
            throw SmeltRigArtifactError.missingFile(url.path)
        }
        ...
        guard let size = attributes[.size] as? NSNumber,
            ...
            throw SmeltRigArtifactError.invalidPackage(
    ...
    /// Returns a zero-copy pointer to a manifest-validated BF16 tensor slice.
    public func checkpointTensorData(
        ...
        precondition(
            ...
            "descriptor does not belong to this rig model artifact"
        )
        let tensor = manifest.tensors[descriptor.index]
        precondition(
            tensor.name == descriptor.name,
            "descriptor name does not match rig model manifest index"
        )
    ...
    public func makeWeightBuffer(device: MTLDevice, tensorNamed name: String) throws -> MTLBuffer {
        guard let tensor = manifest.tensors.first(where: { $0.name == name }) else {
            throw SmeltRigArtifactError.missingTensor(name)
        }
        ...
        guard let buffer = device.makeBuffer(
            ...
            throw SmeltRigArtifactError.cannotCreateWeightBuffer(name)
        }
        buffer.label = "rig.weight.\(name)"
        return buffer
    }
    ...
    private static func verify(_ url: URL, expected: String) throws {
        ...
        guard actual == expected else {
            throw SmeltRigArtifactError.checksumMismatch(
...
public enum SmeltRigArtifactError: Error, Equatable {
