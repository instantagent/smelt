 ...
 * `llg_free_bytes()`. Pass the bytes to `llg_new_tokenizer_from_bytes()` to
 * reconstruct a tokenizer without re-building the trie from the vocabulary.
 * The encoding is native-endian: treat it as a same-architecture cache, not
 * a portable interchange format.
 *
 * Added by ia's tools/llguidance-serialize.patch.
 */
uint8_t *llg_tokenizer_to_bytes(const struct LlgTokenizer *tok, size_t *dst_size);

/**
 * Free a buffer returned by `llg_tokenizer_to_bytes()`. `size` must be the
 * exact value written to `dst_size` by that call.
 */
void llg_free_bytes(uint8_t *ptr, size_t size);

/**
 * Create a tokenizer from trie bytes produced by `llg_tokenizer_to_bytes()`.
 *
 * The vocabulary fields of `tok_init` (`vocab_size`, `tok_eos`, `token_lens`,
 * `token_bytes`, `tokenizer_json`, `tok_eos_extra*`) are ignored — the
 * serialized trie already carries them. The `tokenize_fn` /
 * `tokenize_user_data` / `tokenize_assumes_string` and `slices` fields are
 * used exactly as in `llg_new_tokenizer_v2()`.
 *
 * Returns NULL on error (including corrupted trie bytes) and writes the
 * message to `error_string`.
 */
struct LlgTokenizer *llg_new_tokenizer_from_bytes(const uint8_t *trie_bytes,
                                                  size_t trie_bytes_len,
                                                  const struct LlgTokenizerInitV2 *tok_init,
                                                  char *error_string,
                                                  size_t error_string_len);
